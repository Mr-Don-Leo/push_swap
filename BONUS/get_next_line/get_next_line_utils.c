...
/*   Updated: 2024/07/05 12:37:23 by mbabayan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
char	*gnl_strjoin(char *conserve, char *buffer)
{
	char	*new_string;
	int		index;
	int		index2;
	...
	if (!conserve && !buffer)
		return (NULL);
	new_string = malloc(sizeof(char) * (gnl_strlen(conserve)
				+ gnl_strlen(buffer) + 1));
	if (!new_string)
		return (NULL);
 ...
 */
char	*gnl_strchr(char *concerve, int c)
{
	int	i;
	...
	if (!concerve)
		return (0);
	if ((char)c == '\0')
		return (&concerve[gnl_strlen(concerve)]);
	while (concerve[i])
	{
...
char	*ft_save(char *conserve)
{
	...
	buffer = malloc(sizeof(char) * ((gnl_strlen(conserve) - index) + 1));
	if (!buffer)
		return (NULL);
	index++;
